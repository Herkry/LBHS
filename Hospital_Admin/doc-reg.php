...
$fName = $lName = $username = $docSpecialty = $docPhone	= $password = $confirm_password = "";
$fName_err = $lName_err = $username_err = $docSpecialty_err = $docPhone_err = $password_err = $confirm_password_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $sql = "SELECT docId FROM doctor WHERE docFname = ?";
        ...
        $lName_err = "Please enter a last name.";
			        ...
			        // Prepare a select statement
        $sql = "SELECT docId FROM doctor WHERE docLname = ?";
	    ...
	    if(empty(trim($_POST["docSpecialty"]))){
        $docSpecialty_err = "Please enter a date of birth.";
        }
		else{
						        // Prepare a select statement
        $sql = "SELECT docId FROM doctor WHERE docSpecialty = ?";
        
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "s", $param_docSpecialty);
            
            // Set parameters
            $param_docSpecialty = trim($_POST["docSpecialty"]);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                mysqli_stmt_store_result($stmt);
                    ...
                    $docSpecialty= trim($_POST["docSpecialty"]);
        ...
        if(empty(trim($_POST["docPhone"]))){
            $docPhone_err = "Please enter a Phone Number.";
            }
            else{
                                    // Prepare a select statement
            $sql = "SELECT docId FROM doctor WHERE docPhone	 = ?";
            
            if($stmt = mysqli_prepare($link, $sql)){
                // Bind variables to the prepared statement as parameters
                mysqli_stmt_bind_param($stmt, "s", $param_docPhone	);
                
                // Set parameters
                $param_docPhone	 = trim($_POST["docPhone"]);
    ...
    if( empty($fName_err) && empty($lName_err) && empty($username_err) &&   empty($docSpecialty_err)  &&  empty($docPhone_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO doctor (docFname, docLname, docSpecialty, docPhone	, docEmail, hospId, docPassword) VALUES (?,?,?,?,?,?,?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssss", $param_fName, $param_lName, $param_docSpecialty, $param_docPhone	 ,$param_username,  $param_hospId, $param_password);
            
            // Set parameters
			$param_fName = $fName;
			$param_lName = $lName;
            $param_username = $username;
            $param_docSpecialty= $docSpecialty;
            $param_docPhone	= $docPhone	;
            $password = rand(15,35);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                //header("location: /LBHS/patMapTest.php");
                
                echo("<script> window.location.replace('/LBHS/patMapTest.php'); </script>");
            } else{
...
<div class="w3-row-padding w3-margin-bottom">
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
			...
			<div class="form-group <?php echo (!empty($docSpecialty_err)) ? 'has-error' : ''; ?>">
                <label>Speciality</label>
                <input type="text" name="docSpecialty" class="form-control" value="<?php echo $docSpecialty; ?>">
			    <span class="help-block"><?php echo $docSpecialty_err; ?></span>
			</div>

            <div class="form-group <?php echo (!empty($docPhone_err)) ? 'has-error' : ''; ?>">
                <label>Telephone Number</label>
            
                <input type="tel" name="docPhone" pattern="[0-9]{3}-[0-9]{2}-[0-9]{3}" class="form-control" value="<?php echo $docPhone ; ?>">
			    <span class="help-block"><?php echo $docPhone_err; ?></span>
			</div>
